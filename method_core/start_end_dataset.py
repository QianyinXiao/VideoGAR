...
class StartEndDataset(Dataset):
    ...
    def __init__(self, dset_name, data_path, desc_bert_path_or_handler, sub_bert_path_or_handler, max_desc_len,
                 ...
                 normalize_tfeat=True, h5driver=None, data_ratio=1.0, tokenizer=None, lm_max_len=30,
...
def _get_fixed_length(batch, model_inputs_keys):
    lengths = []
    for key in ("video_feat", "sub_feat", "tef_feat"):
    ...
    for k in model_inputs_keys:
        if "feat" in k:
            if k in ['video_feat', 'sub_feat', 'tef_feat']:
                pad_len = fixed_length
            else:
                pad_len = None
            batched_data[k] = pad_sequences_1d([e["model_inputs"][k] for e in batch], dtype=torch.float32,
                                               fixed_length=pad_len)
        elif k in ["query_input_ids", "query_attn_mask"]:
            dtype = torch.long
            padded, _ = pad_sequences_1d([e["model_inputs"][k] for e in batch], dtype=dtype)
